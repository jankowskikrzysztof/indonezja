...
{
    location.href = 'report_activities_year.php?senkut=<?php echo $_GET['senkut']; ?>&period=' + url;
}
</script>
...
echo '<h2 align=center>Annual Activities Report: '.$period_year.'</h2>';
...
$stmt = $dbh->prepare("SELECT activity.* FROM activity
...
and cash_book_item.activity_id=".$row['id_activity']."
			...
			".$location_limit_sel."
			".$period_limit."
GROUP BY YEAR(cash_book.date), MONTH(cash_book.date), cash_book_item.activity_id
